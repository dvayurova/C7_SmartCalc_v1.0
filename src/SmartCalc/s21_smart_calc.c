...
int parse_operator(char *str, int *i, operators **oper_stack,
                   numbers **num_stack) {
  int res = 0;
  if (str[*i] == '+') push_logic(i, oper_stack, num_stack, 1, '+', PLUS, 2);
  if (str[*i] == '-') push_logic(i, oper_stack, num_stack, 1, '-', MINUS, 2);
  if (str[*i] == '*') push_logic(i, oper_stack, num_stack, 1, '*', MULT, 2);
  if (str[*i] == '/') push_logic(i, oper_stack, num_stack, 1, '/', DIV, 2);
  if (str[*i] == '^') push_logic(i, oper_stack, num_stack, 1, '^', EXP, 2);
